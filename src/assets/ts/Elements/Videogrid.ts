export class Videogrid {
  recalculateLayout() {
    ...
    var aspectRatio = 4 / 3;
    if (videoCount <= 2) {
      var lastElementVideo = $(gallery)
        .find('.video-item:not(.hide)')
        .last()
        .find('video')[0];
      if (
        lastElementVideo !== undefined &&
        lastElementVideo.videoWidth > 0 &&
        lastElementVideo.videoHeight > 0
      ) {
        aspectRatio =
          lastElementVideo.videoWidth / lastElementVideo.videoHeight;
      }
    }
